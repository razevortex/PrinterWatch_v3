...
class CartView(object):
    def __init__(self, **kwargs):        
        ...
        print(kwargs)
        self.selectable = db.cartridges.name_index
        self.x = 0
        self._selecting(**kwargs)
        print(self.x)
        obj = db.cartridges.obj[self.x]
    ...
    def _selecting(self, **kwargs):
        ...
        if not kwargs.get('select', False) and not kwargs.get('cart_id', False):
            ...
            if kwargs.get('select', False) != kwargs.get('cart_id', False):   # if select dropdown was changed use selected objs index
                self.x = db.cartridges.name_index.index(kwargs.get('select'))
            else:
                self.x = db.cartridges.name_index.index(kwargs.get('cart_id'))  # else use the previous objs index
                self._saveing(**kwargs)
        self.x += int(kwargs.get('next', False) == 'Next') - int(kwargs.get('back', False) == 'Back')
        self.x = self.x + len(db.cartridges.name_index) if self.x < 0 else self.x % len(db.cartridges.name_index)
    ...
    def _saveing(self, **kwargs):
        ...
        if kwargs.get('reset', False) == '1':
            temp['reset'] = True
        db.cartridges.obj[self.x].edit(**temp)
        #db.cartridges.save()
